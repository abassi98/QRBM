...
contains
  ...
  double complex function Heisenberg_loc(self, S, pbc)
    ...
    do  ii=1,self%Nv-1
       ...
       ! Take the difference of exponential part
       frac = exp(-2.*sum(self%a)*(S(ii)+S(ii+1))) 
       ...
       do ss=0,self%Nv-1
          ...
          ! Compute permutation index realtive to ii
          iph1 = mod(ii+ss-1,self%Nv)+1

          ! Compute permutation index realtive to ii+1
          iph2 = mod(ii+ss,self%Nv)+1
          
          ! Compute theta of new configuration
          theta_new = theta - 2.*self%W(iph1,:)*S(iph1) - 2.*self%W(iph2,:)*S(iph2)
    ...
    if (pbc .eqv. .true.) then
       ...
       ! Take the difference of exponential part
       frac = exp(-2.*sum(self%a)*(S(1)+S(self%Nv))) 
       ...
       do ss=0,self%Nv-1
          ...
          ! Compute permutation index realtive to 1
          iph1 = mod(ss,self%Nv)+1

          ! Compute permutation index realtive to self%Nv
          iph2 = mod(self%Nv+ss-1,self%Nv)+1
